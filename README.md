...
## How To Use Pocket Curator

When Pocket Curator opens it will show the system carousel - all your systems, just like EmulationStation does. Upon opening, it is in DELETE MODE. Which means if you go into a system, you can mark files for deletion. Use the system carousel to scroll through your systems. Press A to enter a system (such as Game Boy Color). It shows your gbc ROMs with screen shot (sorry, no video) and rating and description. Scroll through the game list just like in EmulationStation. Pressing A will mark a game to be deleted. Pressing X will start the delete. It will show you a list of what's doing to be deleted and how much space you'll free up, and it asks for confirmation or cancel. If you confirm, it deletes the ROM and the associated scraped files for that game. Hit the B button to go back to the system carousel. Hit the Start button to toggle Select All or Select None. Press the Select button to bring up a Settings menu.

If you want to change to FETCH MODE (to copy games from a WebDAV server), from the system carousel scroll to the system you want to copy games for, then press the Y button. That will bring up a menu where you can scan your local network for a WebDAV server, or manually enter an IP address (or DNS name) and port number of a server. (It remembers servers you've connected to before, and will show them on the menu so you don't have to type in all that stuff again!) A typical server address will look like this: 192.168.1.22:5005 or roms.myserver.org:5005. Once you enter the server address it will connect and it will look at the folders you have there and attempt to jump right into the system folder for the system you pressed Y on a moment ago. Once it connects it looks for a gamelist.xml file in that folder and if it finds it, it lists only the games in that gamelist.xml file. As you scroll through the games, it will show the screen shot and metadata (just like in DELETE MODE), but this info comes from your server. There's a slight delay between scrolling and it displaying the screen shot and metadata (again, because it's pulling this info from your server). It also shows subfolders at the top (which you probably have if these ROMs have been scraped). You can go into a folder by highlighting it and pressing the A button. Go back up a folder by pressing the B button. Select the games you want to copy by highlighting them and pressing the A button. Either a yellow ? or a green + will appear next to them. The yellow ? indicates that Pocket Curator sees you already have this game on your handheld. A green + means you don't have it on your handheld (yet). Once you select all the games you want to copy, press the X button. Then it will check for scraped files to go with those games and tally up how much space it needs on your card to copy them all. It will let you copy just the ROMs if you want (no scraped files), and it will let you copy just the games you DON'T have yet, and it will let you copy and overwrite the ones you have already. You've got options. Once you make your selection, the copy process starts. It will go pretty quickly (faster than SAMBA copies), and will show a progess bar across the bottom of the screen. You can select and add more games to the copy queue even while it is still copying. So feel free to back out of the system folder you're in and go to another system folder. Pocket Curator will figure out where those ROM files go on your handheld - you do NOT need to exit (disconnect from the server) and go back in to select a different system. Just hit B to go up a folder, select your new folder and press A to go into that folder. Select more games, press X and add them to what's already being copied. Press B a couple of times to back your way out and disconnect from the server (returning to DELETE MODE). That won't stop a copy queue if it is still copying. The copy will continue until its finished. You can't exit Pocket Curator while it's working on copying. It will warn you.

Lastly, you can add ROMs to system that do NOT show up on your system carousel. Just pick any system (or maybe your device is a fresh install and is empty) press Y to open FETCH MODE and it will go through the regular server connection prompts. Once you're connected to your server, you navigate to any folder on your WebDAV server (in that share) and copy to your device. Pocket Curator will try to figure out where the ROMs need to go and put them in the right place - even if the roms folder structure is from a different firmware. If it can't figure it out on its own, it will ask you what folder you want them put in.

## Controls

**Systems carousel:**
   - Up/Down/Left/Right - scroll through your game systems
   ...
   - Select - Settings Menu (you can update Pocket Curator from here, change the font size, and more)
